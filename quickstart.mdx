...
description: "Start quickly with Nimbus by choosing the Python or Julia SDK and following setup paths for your first BCI workflow."
...
<CardGroup cols={2}>
  <Card title="Python SDK Quickstart" icon="python" href="/python-sdk/quickstart">
    Get started with `nimbus-bci` (fully local)
  </Card>
  <Card title="Julia SDK Quickstart" icon="code" href="/julia-sdk/quickstart">
    Get started with NimbusSDK.jl (requires API key)
  </Card>
</CardGroup>

<Note>
If you landed here from an older link: the Julia quickstart moved from `/quickstart` to `/julia-sdk/quickstart`.
</Note>

## Start Here

<CardGroup cols={2}>
  <Card title="Python Setup Path" icon="python" href="/python-sdk/quickstart">
    Install `nimbus-bci`, run your first classifier, and add streaming.
  </Card>
  <Card title="Julia Setup Path" icon="code" href="/julia-sdk/quickstart">
    Install NimbusSDK.jl, authenticate, and run your first inference.
  </Card>
  <Card title="Preprocessing Requirements" icon="list-filter" href="/inference-configuration/preprocessing-requirements">
    Required feature pipeline for motor imagery, P300, and SSVEP.
  </Card>
  <Card title="Model Selection Guide" icon="brain" href="/model-specification">
...
- **Python 3.11+** (for Python SDK) or **Julia 1.9+** (for Julia SDK)
...
<Tip>
**Important**: Nimbus SDKs expect **preprocessed features**, not raw EEG data. Apply filtering, artifact removal, and feature extraction (for example, CSP) before inference.
</Tip>
...
<AccordionGroup>
  <Accordion title="Do I need an API key for both SDKs?" icon="key">
    No. The Python SDK (`nimbus-bci`) runs locally and does not require an API key. The Julia SDK (`NimbusSDK.jl`) requires an API key for installation and model access.
  </Accordion>
  <Accordion title="What data format does Nimbus expect?" icon="database">
    Use extracted features, not raw EEG. Typical shape is `(n_features x n_samples x n_trials)` for batch workflows. See [Preprocessing Requirements](/inference-configuration/preprocessing-requirements) for full guidance.
  </Accordion>
  <Accordion title="What should I read next after setup?" icon="arrow-right">
    Continue with [Streaming Inference](/inference-configuration/streaming-inference), [Feature Normalization](/inference-configuration/feature-normalization), and [Basic Examples](/examples/basic-examples).
  </Accordion>
</AccordionGroup>
