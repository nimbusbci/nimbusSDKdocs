...
- `:posterior_mean`: Average chunk posteriors
- `:unanimous`: Require all chunks to agree, falling back to weighted vote on disagreement
...
Average chunk-level posterior distributions:

```julia
final = finalize_trial(session; method=:posterior_mean)
```

**Best for:** Preserving full posterior evidence across chunks
...
Require all chunks to agree, with fallback to weighted voting if they disagree:

```julia
final = finalize_trial(session; method=:unanimous)
```

**Best for:** Conservative decisions when chunk-level agreement matters
