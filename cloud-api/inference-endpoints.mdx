...
<AccordionGroup>
  ...
  <Accordion title="Why can model loading be fast after first use?" icon="clock">
    Models are cached locally after the first download, so subsequent `load_model(...)` calls typically avoid network fetches and complete much faster.
  </Accordion>
</AccordionGroup>
