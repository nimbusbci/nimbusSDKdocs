---
title: "Julia SDK API Reference"
description: "Complete API reference for NimbusSDK.jl covering NimbusLDA, NimbusQDA, NimbusProbit, and RxInfer-powered BCI workflows."
...
- `name::String` - Model name (defaults are model-specific: `"custom_nimbus_lda"`, `"custom_nimbus_qda"`, or `"custom_nimbus_probit"`)
...
- `label::Union{Int, Nothing}` - Optional ground-truth label for calibration metrics
...
struct StreamingResult
  ...
  chunk_posteriors::Vector{Vector{Float64}}  # Per-chunk posterior distributions
...
**Parameters:**
- `features::Matrix{Float64}` - Feature matrix (n_trials × n_features)
...
aggregate_chunks(
    ...
    n_classes::Int;
    posteriors::Union{Vector{Vector{Float64}}, Nothing} = nothing,
    method::Symbol = :weighted_vote
) -> NamedTuple
```
...
- `n_classes::Int` - Number of classes
- `posteriors` - Optional full posterior distributions from each chunk
- `method::Symbol` - Aggregation method (`:weighted_vote`, `:max_confidence`, `:posterior_mean`, `:unanimous`)
