...
- **Python SDK**: Uses optimized local Python/scientific-computing implementations, with optional JAX support for `NimbusSoftmax`
...
print(f"ITR: {full_metrics['itr']:.1f} bits/min")
...
# Assume trial_features is the feature array for the current trial
confidence = 0.65
trial_features = X_test[0]
...
if not quality.is_valid:
    print(f"Trial rejected: {quality.rejection_reason}")

# Check if rejection is warranted
if should_reject_trial(confidence, threshold=0.7):
    print("Low confidence trial - consider rejecting")
else:
...
<Note>
**Next**: Learn how Nimbus handles [uncertainty in neural signals](/core-concepts/probabilistic-ai) to build robust BCI applications.
</Note>
