...
Both SDKs use Bayesian inference to model uncertainty explicitly with production-ready heads: Bayesian LDA, Bayesian QDA, Bayesian Softmax, and Bayesian STS (Python SDK). In Python apps, prefer [`Personalizer` → `BrainState`](/personalizer/overview); use standalone heads for research diagnostics.
