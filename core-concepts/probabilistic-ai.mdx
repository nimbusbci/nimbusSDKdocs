...
Both SDKs use Bayesian inference to model uncertainty explicitly with production-ready heads: Bayesian LDA, Bayesian QDA, Bayesian Softmax, and Bayesian STS (Python SDK). In Python apps, prefer [`Personalizer` → `BrainState`](/python-sdk/middleware); use standalone heads for research diagnostics.
...
for state in states:
    print(state.intent, state.confidence, state.uncertainty)
    if state.need_more_data or state.rejected:
        print("Gate: ask for more data / reject trial")
```
  </Tab>
