...
<Note>
**Python head:** prefer standalone `NimbusSTS` / streaming sessions for latent-state adaptation. Personalizer currently ships `lda` / `qda` / `softmax` heads; STS remains the classical path for non-stationary sessions. See [Personalizer & Middleware](/python-sdk/middleware).
</Note>
