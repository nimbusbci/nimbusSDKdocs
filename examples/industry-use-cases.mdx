...
<Tip>
Shipping an app on a frozen partner / foundation encoder? Lead with [Personalizer → BrainState](/personalizer/overview). The head choices below (`NimbusLDA`, …) are what Personalizer uses under the hood (`head="lda"|...`) and also work standalone on classical features.
</Tip>
...
| Motor imagery rehab | `NimbusLDA` / Personalizer | Streaming with confidence / `need_more_data` gates. |
