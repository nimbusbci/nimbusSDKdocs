...
Product path: wrap any `encode(X) → Z`, personalize online, consume `BrainState`.
...
Already have CSP / bandpower features? Use `Personalizer(encoder=None)` or the classical heads below. Full contract: [Personalizer & Middleware](/personalizer/overview).
