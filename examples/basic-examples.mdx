...
<Note>
  ...
  - Paradigms: [Paradigms & profiles](/personalizer/paradigms)
...
adapter = Personalizer.for_deployment(enc, ["left", "right"])
adapter.fit(X_cal, y_cal)
states = adapter.predict(X_test)
print(states[0].intent, states[0].confidence, states[0].need_more_data)
```

Already have CSP / bandpower features? Use `Personalizer.for_features(["left", "right"])` — same app surface. Full contract: [Personalizer](/personalizer/overview).
...
adapter = Personalizer.for_features(["left", "right"], head="lda")
adapter.fit(X, y)
...
Preferred product path (`pip install nimbus-bci[riemann]`): xDAWN → tangent via `Personalizer.for_p300`. Classical ERP amplitude features still work with `for_features`.

<Tabs>
  <Tab title="Python (for_p300)">
```python
from nimbus_bci import Personalizer

# epochs: target/non-target MNE epochs (raw-ish ERP windows)
p = Personalizer.for_p300(
    ["NonTarget", "Target"],
    tracking_alpha=0.05,  # opt-in geodesic tracking
)
p.fit(epochs_cal, y_cal)  # see fit_p300_session for day-2 bundles
states = p.predict(epochs_test)
print(states[0].intent, states[0].confidence)
```
  </Tab>
  <Tab title="Python (ERP features)">
    ```python
    from nimbus_bci import Personalizer
    ...
    adapter = Personalizer.for_features(["nontarget", "target"], head="qda")
    adapter.fit(X, y)
...
| --- | --- | --- |
| `Personalizer.for_deployment` + `wrap` | Frozen / foundation encoder | Product wedge → `BrainState` |
| `Personalizer.for_features` | Classical CSP / bandpower / ERP features | Same app surface; pick `head=` |
| `Personalizer.for_riemann` / `for_p300` | Weak-Z / P300 geometry (`[riemann]`) | Geodesic tracking + session bundles |
...
<Columns cols={2}>
  ...
  <Card title="Paradigms & profiles" icon="route" href="/personalizer/paradigms">
    MI / P300 session bundles and day-2 profiles
  </Card>
  <Card title="Python SDK Quickstart" icon="braces" href="/python-sdk/quickstart">
