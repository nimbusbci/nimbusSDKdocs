...
clf_qda = NimbusQDA(mu_scale=3.0)
...
# Or train NimbusQDA model (more flexible, class-specific covariances)
qda_model = train_model(
    NimbusQDA,
    train_data;
    iterations = 50,
    showprogress = true,
    name = "my_motor_imagery_qda"
)
...
<Tabs>
  <Tab title="Python">
```python
from nimbus_bci import NimbusQDA  # QDA often works well for P300
import numpy as np
import mne
...
# Train classifier (QDA handles P300's overlapping distributions well)
clf = NimbusQDA()
...
for clf, name in [(clf_lda, "NimbusLDA"), (clf_qda, "NimbusQDA")]:
...
println("\nTraining NimbusQDA...")
qda_model = train_model(NimbusQDA, train_data; iterations=50, showprogress=true)
...
for (model, name) in [(rxlda, "NimbusLDA"), (qda_model, "NimbusQDA")]
