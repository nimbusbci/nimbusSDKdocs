...
description: "Full nimbus-bci Python API reference for Personalizer, BrainState, wrap, NimbusLDA, NimbusQDA, NimbusSoftmax, NimbusSTS, and core inference utilities."
keywords: ["API reference", "Python SDK", "nimbus-bci", "Personalizer", "BrainState", "NimbusLDA", "NimbusQDA", "NimbusSoftmax", "NimbusSTS", "BCI API"]
icon: "book"
---
...
Product surface: frozen trunk → Bayesian head → `BrainState`. Narrative guide: [Personalizer & Middleware](/personalizer/overview).
...
# Preferred entry points (0.6):
adapter = Personalizer.for_deployment(enc, ["left", "right"], paradigm="motor_imagery")
# adapter = Personalizer.for_research(enc, ["left", "right"], transform="whiten")
# adapter = Personalizer.for_features(["left", "right"])
# adapter = Personalizer.for_riemann(["left", "right"])   # needs [riemann]
# adapter = Personalizer.for_p300(["NonTarget", "Target"])  # needs [riemann]
```

**Factories:** `for_deployment`, `for_research`, `for_features`, `for_riemann`, `for_p300`, `for_balora` (needs `[train]`).

**Methods:**
- `fit(X, y, *, input_sfreq=None)` — calibrate on labeled trials
- `fit_calibrated(X, y, X_val, y_val, *, target_accept=...)` — fit + pin decision-gate accept rate
- `partial_fit(X, y, classes=None)` — cumulative online head update
- `adapt(X, y=None, level=...)` — label-agnostic adaptation gateway
- `adapt_async(...)` — shadow trunk adaptation with atomic commit
- `predict(X)` → `list[BrainState]` or `BrainStateBatch`
- `predict_one(x)` → `BrainState`
- `export_profile()` / `Personalizer.from_profile(profile, encoder=...)` — portable profile
- `recommend_adapt(X_stream, *, tau, X_ref=None)` → `AdaptDecision` — thin mean-shift helper (L0 vs L1 only; **user-supplied** `tau`)
...
Also exported: `Recipe`, `UserProfile`, `BrainStateBatch`, `recommend_adapt`, `embedding_mean_shift`, `AdaptDecision`, transform types.

**Paradigm helpers** (see [Paradigms & profiles](/personalizer/paradigms)):

- `fit_mi_session` / `calibrate_mi_bald` / `MISessionBundle` — MI geometry gateway + day-2 bundle
- `fit_p300_session` / `P300SessionBundle` — xDAWN→tangent + day-2 bundle (`trusted=True` on load)
- Session bundles verify `sidecar_sha256` before unpickling joblib sidecars

**Also see:** [Transforms](/personalizer/transforms) · [Research / PEFT](/personalizer/research-peft) · [Changelog 0.6](/python-sdk/changelog-0.6)

**Notes:**
- Prefer factories over raw constructors for common cases
- Decision presets: `"strict"` / `"permissive"` / `"aggressive"` (0.6 rename; was `research` / `consumer`)
- `encoder=None` / `for_features` uses classical features already shaped `(n, d)`
- `head="softmax"` requires `pip install nimbus-bci[softmax]`
- Default `partial_fit` is cumulative (`λ=1`); optional forgetting uses `λ\<1` on LDA/QDA
- After `load` / `from_profile`, pass `X_ref=` to `recommend_adapt` when needed
- Trial rejection (`BrainState.rejected`) ≠ adapt recommendation (`should_adapt`)

### wrap / FrozenEncoder

```python
from nimbus_bci import wrap

enc = wrap(
    model.encode,              # callable | sklearn transformer | torch Module
    model_id="partner-encoder",
    embedding_dim=64,          # optional width check
)
Z = enc.encode(X)              # (n, d)
```
...
App-facing decision object.

**Fields:** `intent`, `confidence`, `posterior`, `uncertainty`, `alternatives`, `need_more_data`, `rejected`, `rejection_reason`, `embedding`, `user_id`, `model_id`, `paradigm`, `extras`

**Methods:** `to_dict()` — JSON-friendly export (`brain_state_version` currently `1`)
...
clf = NimbusLDA(
    ...
    # 0.6: prior_fraction= / forgetting= on LDA/QDA — see parameters below
)
```
...
- `prior_fraction` (float or None, default=None on the classifier; product heads often use `0.05`): Conjugate prior weight as a fraction of `n` (`None` = identity prior)
- `forgetting` (float, default=1.0): Recency factor on online updates; `λ\<1` optional (`λ=1` is byte-identical to cumulative)
...
auc = roc_auc_score(y_true, y_score)  # see docstring for multi-class shapes
brier = brier_score(y_true, y_proba)
scaled = temperature_scale_logits(logits, temperature=T)
```

Also: artifact-robust `trial_quality` scoring knobs used with Personalizer evaluation.
...
<AccordionGroup>
  <Accordion title="Which classifier should I start with?" icon="circle-question-mark">
