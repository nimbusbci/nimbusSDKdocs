...
adapter = Personalizer(
    encoder=enc,          # FrozenEncoder | callable | None
    head="lda",           # "lda" | "qda" | "softmax" | sklearn-like object
    user="subject_001",
    classes=["left", "right"],
    preset="research",    # "research" | "consumer" | None
    confidence_threshold=0.6,
    uncertainty_threshold=0.55,
    paradigm=None,
    mc_samples=None,      # >=2 → posterior sampling (BALD)
    mc_rng_seed=0,
)
```
...
Also exported: `recommend_adapt`, `embedding_mean_shift`, `AdaptDecision` (standalone; same thin helper).
