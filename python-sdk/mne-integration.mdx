...
2. **Classical:** epochs → CSP / bandpower → `NimbusLDA` (examples below)
</Tip>

```python
from nimbus_bci import Personalizer, wrap

# After you have trial tensors X for a frozen trunk
...
## Basic Workflow (classical CSP path)
...
For workflows that use covariance geometry, install the pyRiemann extra and build a sklearn pipeline from EEG epochs to a Nimbus classifier head:
