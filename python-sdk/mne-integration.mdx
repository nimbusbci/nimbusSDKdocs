...
description: "Use MNE-Python with nimbus-bci: epochs → frozen encoder → Personalizer → BrainState, or classical CSP → Nimbus heads."
keywords: ["MNE-Python", "EEG preprocessing", "Python SDK", "nimbus-bci", "Personalizer", "BrainState", "CSP", "bandpower", "ERP", "MNE integration"]
icon: "brain"
---

# MNE-Python Integration

The nimbus-bci library integrates seamlessly with MNE-Python, the leading Python package for EEG/MEG analysis. Build complete BCI pipelines from raw EEG to `BrainState` decisions.
...
1. **Product:** epochs → frozen encoder (`wrap` / BrainDecode) → [`Personalizer`](/python-sdk/middleware) → `BrainState`
...
# Trial tensor for your frozen trunk — shape depends on encode()
X = epochs.get_data()  # (n_epochs, n_channels, n_times)
y = epochs.events[:, 2]
# Map to semantic labels if needed
classes = ["left", "right"]
...
adapter.save("subject_mne_profile")
```

Optional shortcuts: `wrap_eegnet`, `wrap_braindecode` — see [Personalizer & Middleware](/python-sdk/middleware).
...
```python
from nimbus_bci import Personalizer, NimbusLDA
from sklearn.model_selection import cross_val_score

labels = epochs.events[:, 2]

# Product-style on classical features
adapter = Personalizer(
    encoder=None,
    head="lda",
    classes=["left", "right"],
...
adapter.fit(csp_features, labels)
states = adapter.predict(csp_features[:5])

# Or standalone head for sklearn CV
clf = NimbusLDA()
...
# 7. Scale + Personalizer (encoder=None on classical features)
...
adapter.fit(Z_train, y_train)
states = adapter.predict(Z_test)
print(states[0].intent, states[0].confidence)

# 8. Evaluate via the head (sklearn-compatible)
train_score = adapter.head.score(Z_train, y_train)
test_score = adapter.head.score(Z_test, y_test)
...
X_epochs = epochs.get_data()  # (n_trials, n_channels, n_times)
y = epochs.events[:, 2]

X_train, X_test, y_train, y_test = train_test_split(
    X_epochs, y, test_size=0.2, stratify=y, random_state=42
)
