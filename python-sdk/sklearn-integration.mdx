...
description: "Use nimbus-bci heads with sklearn pipelines, CV, and GridSearch — alongside Personalizer for app-facing BrainState outputs."
keywords: ["sklearn", "scikit-learn", "Python SDK", "nimbus-bci", "Personalizer", "pipelines", "cross-validation", "hyperparameter tuning", "sklearn integration"]
icon: "code"
---

The nimbus-bci **heads** are fully compatible with scikit-learn — pipelines, cross-validation, hyperparameter tuning, and the broader sklearn ecosystem. Use them standalone or as `Personalizer(head=...)` under the [middleware contract](/personalizer/overview).

<Tip>
**Product vs sklearn surface**
- Apps / devices: `Personalizer` → `BrainState` ([middleware](/personalizer/overview)).
...
Z_train = enc.encode(X_train)
Z_test = enc.encode(X_test)

# App path — BrainState on already-encoded embeddings
adapter = Personalizer.for_features(["left", "right"], head="lda")
adapter.fit(Z_train, y_train)
states = adapter.predict(Z_test)

# Research path — CV on the same head
scores = cross_val_score(adapter.head, Z_train, y_train, cv=5)
...
# Create pipeline — head is also Personalizer(head="lda") under the hood
pipe = make_pipeline(
    StandardScaler(),
...
# Select best
best_head = max(results.items(), key=lambda x: x[1].mean())
print(f"\nBest head: {best_head[0]}")
```
