...
- Research CV / GridSearch: use the head directly (`NimbusLDA`, …) or `adapter.head` after fitting a Personalizer.
</Tip>
...
```python
from nimbus_bci import NimbusLDA, Personalizer, wrap
...
# Personalizer wraps the same head after encode
enc = wrap(lambda X: X, model_id="demo", embedding_dim=X_train.shape[1])
adapter = Personalizer(encoder=enc, head="lda", classes=["left", "right"])
adapter.fit(X_train, y_train)
# GridSearchCV / CV typically target adapter.head on already-encoded Z
```
