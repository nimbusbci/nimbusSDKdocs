...
<Note>
  This page covers the **Python API** (`StreamingSession`, aggregation, BrainState bridges). For shared chunk-size and aggregation concepts across SDKs, see [Streaming inference configuration](/inference-configuration/streaming-inference).
</Note>

## Overview

Streaming inference lets you process EEG as it arrives and predict on partial trials before a full trial finishes — useful when latency matters.
...
For app integration, convert a finalized `StreamingResult` into a `BrainState`:

```python
from nimbus_bci import NimbusLDA, StreamingSession, brain_state_from_streaming_result

clf = NimbusLDA().fit(X_cal, y_cal)
session = StreamingSession(clf, n_features=X_cal.shape[1], chunk_size=25)

# ... process_chunk(...) for the trial ...
final = session.finalize_trial()
...
Prefer `BrainState` when shipping device / UI decisions. Keep `StreamingResult` for research diagnostics. See [BrainState](/personalizer/brain-state).
