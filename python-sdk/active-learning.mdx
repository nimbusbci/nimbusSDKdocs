...
<Info>
  Active learning answers **which trials to label**. That is different from `Personalizer.adapt` / `recommend_adapt` (whether / how to update online) and from `BrainState` trial rejection. See [Personalizer overview](/personalizer/overview).
</Info>

<Note>
  Active learning operates on **preprocessed feature rows** (or already-encoded embeddings), not raw EEG. Use arrays shaped `(n_trials, n_features)` for pools and `(n_features,)` or `(1, n_features)` for single streaming trials. With Personalizer, prefer the wrappers `adapter.suggest_calibration_trials(...)` / `adapter.calibration_status(...)` (they encode first), or run AL on `adapter.head` with a pre-encoded pool.
</Note>

## With Personalizer

Compose active learning with the product surface: choose labels, then update via `adapt` / `partial_fit`.
...
# Personalizer wrappers encode the pool for you
...
status = adapter.calibration_status(
    X_probe,  # never-labeled probe set
    criterion="posterior_stability",
    previous=None,  # or a prior Personalizer / head snapshot
    threshold=0.02,
)

# Or CalibrationSession on the head with a pre-encoded pool
