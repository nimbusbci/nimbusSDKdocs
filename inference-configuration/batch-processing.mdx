...
<Note>
Both SDKs support Bayesian LDA and Bayesian QDA. Python additionally supports `NimbusSoftmax` and `NimbusSTS`; Julia additionally supports `NimbusProbit`. Examples show both SDKs - you can use any model type.
</Note>
...
for (pred, true_label, conf) in zip(results.predictions, labels, results.confidences)
    ...
    if length(tracker.predictions) % 10 == 0
...
qda_model = train_model(NimbusQDA, train_data; iterations=50)

# Test both models
models = [rxlda, qda_model]
