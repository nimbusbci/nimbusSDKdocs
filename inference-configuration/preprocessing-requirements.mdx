...
1. **Product path:** freeze a neural trunk → `wrap` → [`Personalizer`](/python-sdk/middleware) → `BrainState` (Nimbus does not own the DL architecture).
...
<Note>
Encoder contract: [Personalizer & Middleware](/python-sdk/middleware). Classical tool integration: [Real-time Setup](/inference-configuration/real-time-setup).
</Note>
