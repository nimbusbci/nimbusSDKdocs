...
qda_model = load_model(NimbusQDA, "motor_imagery_4class_qda_v1")
# Typical latency: 15-25ms
...
### Posterior Mean
```julia
# Average chunk posteriors
final_result = finalize_trial(session; method=:posterior_mean)
```
- Best for: Preserving probabilistic evidence across chunks
- Advantage: Uses the full posterior distribution from each chunk
- Typical accuracy: Good when posteriors are well-calibrated

### Max Confidence
```julia
# Use the most confident chunk
